...
public class FoodTruckApplication extends Application {
  ...
  private GridPane getFoodList() {
    ...
    // grid.setConstraints(child, columnIndex, rowIndex, columnspan, rowspan, halignment,
  ...
  private GridPane getMealGrid() {
    ...
    // TODO: right offset of 100 gets things closer together, but I need to set each layout (2
    // GridPane, 1 BorderPane) to move with each other
    mealGrid.setPadding(new Insets(10, 100, 10, 10)); // right offset of 100 gets things closer
                                                      // together
    ...
    // if a ListView of type String, we can display the name of each meal; if a ListView of type
    // Meal, we can dynamically change nutrientField
    ListView<Meal> mealListView = new ListView<Meal>();
    mealListView.getSelectionModel().selectionModeProperty().set(SelectionMode.SINGLE); // not sure
                                                                                        // if I need
                                                                                        // to list
                                                                                        // this -- I
                                                                                        // think
    ...
    /*
     * // TODO: Is there a better way of implementing this instead of regenerating nutrientString
     * and re-running analyzeMealData with each selection? // Use listeners to update mealListView
     * whenever a new Meal is selected // Build nutrientString for newValue (the newly selected
     * meal), then set this value to nutrientField
     * mealListView.getSelectionModel().selectedItemProperty().addListener( (observable, oldValue,
     * newValue) ->
     * nutrientField.setText(mealListView.getSelectionModel().getSelectedItem().analyzeMealData())
     * ); // Clear out nutrientString for newValue so that the next time that same Meal is selected,
     * it is starting with a clean nutrientString // (instead of appending to a nutrientString that
     * already mealListView.getSelectionModel().selectedItemProperty().addListener( (observable,
     * oldValue, newValue) -> newValue.cleanUpSring() );
     */

    // Creates a listener to update the value in the TextArea (nutrientField) whenever a value is
    // selected in the meal list (mealListView).
    mealListView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Meal>() {
      ...
      public void changed(ObservableValue<? extends Meal> observable, Meal oldValue,
        ...
        if (newValue.getNutrientString().isEmpty()) {
          nutrientField
              .setText(mealListView.getSelectionModel().getSelectedItem().analyzeMealData());
        } else {
